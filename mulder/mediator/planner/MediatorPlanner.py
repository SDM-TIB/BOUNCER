...
class MediatorPlanner(object):
    ...
    def includePhysicalOperators(self, tree):
        ...
        if isinstance(tree, Leaf):
            ...
            if (tree.filters == []):
                return self.includePhysicalOperatorJoin(left_subtree, right_subtree, tree.join_type)
            else:
                n = self.includePhysicalOperatorJoin(left_subtree, right_subtree, tree.join_type)
                for f in tree.filters:
                    vars_f = f.getVarsName()
                    if set(n.vars) & set(vars_f) == set(vars_f):
                        n = TreePlan(Xfilter(f), n.vars, n)
    ...
    def includePhysicalOperatorJoin(self, l, r, join_type='I'):
    ...
    def joinIndependent(self, l, r):
        ...
        if isinstance(l, IndependentOperator) and l.tree.service.triples[0].subject.constant:
            ...
            n = TreePlan(Xgjoin(join_variables), all_variables, l, r)
        elif not lowSelectivityLeft and not lowSelectivityRight and (not isinstance(l, TreePlan) or not isinstance(r, TreePlan)):
            ...
            if len(join_variables) > 0:
                ...
                #         n = TreePlan(NestedHashJoin(join_variables), all_variables, r, l)
                #         dependent_join = True
        elif not lowSelectivityLeft and lowSelectivityRight and not l.__class__.__name__ == "NestedHashJoinFilter" and not (r.__class__.__name__ == "NestedHashJoinFilter" or r.__class__.__name__ == "Xgjoin"):
            if len(join_variables) > 0:
                n = TreePlan(NestedHashJoin(join_variables), all_variables, l, r)
                dependent_join = True
        elif lowSelectivityLeft and not lowSelectivityRight and not r.__class__.__name__ == "NestedHashJoinFilter" and not (l.__class__.__name__ == "NestedHashJoinFilter" or l.__class__.__name__ == "Xgjoin"):
            if len(join_variables) > 0:
                n = TreePlan(NestedHashJoin(join_variables), all_variables, r, l)
                dependent_join = True
