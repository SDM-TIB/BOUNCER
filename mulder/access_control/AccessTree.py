...
class Node(Tree):
    ...
    def instantiate(self, d):
        return Node(self.left.instantiate(d), self.right.instantiate(d), join_type=self.join_type)

    def instantiateFilter(self, d, filter_str):
        return Node(self.left.instantiateFilter(d, filter_str), self.right.instantiateFilter(d, filter_str), join_type=self.join_type)
    ...
    def __eq__(self, other):
        return ((isinstance(other, Node)) and (self.vars == other.vars) and
                ...
                (self.right == other.right) and (self.join_type == other.join_type))
...
class Leaf(Tree):
    def __init__(self, s, vs, dc, filter=[],molecules=[], datasets=[], independent=True):
        self.vars = vs
        self.dict = dc
        self.size = 1
        self.service = s
    ...
    def __hash__(self):
        mols = "-".join(self.molecules)
        ds = "-".join(self.datasets)
        return hash((self.vars, self.dict, self.size, self.degree(),
                     self.service, mols, ds, self.independent))
    ...
    def __eq__(self, other):
        return ((isinstance(other, Leaf)) and (self.vars == other.vars) and
                ...
                len(set(self.molecules).intersection(set(other.molecule))) == len(set(self.molecules)) and
                len(set(self.datasets).intersection(set(other.datasets))) == len(set(self.datasets)) and
    ...
    def instantiate(self, d):
        ...
        return Leaf(self.service.instantiate(d), newvars, newdict,
                    molecules=self.molecules, datasets=self.datasets, independent=self.independent)
    ...
    def getInfoIO(self, query):
        ...
        # variables = [str.lstrip(str.lstrip(v, "?"), "$") for v in vs]
        if query.args == []:
            projvars = vs
        else:
        ...
        if query.distinct:
            ...
            d = ""
        # TODO: if this leaf is Not independent, then put restriction on rdf:type ?type, Filter (?type =<self.molecules[i]>)
        # TODO: if query modification is allowed, then remove subvars (projection) that are not permitted for this leaf in this service.endpoint
