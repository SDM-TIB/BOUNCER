...
def get_rdfs_ranges(server, p, limit=-1):
    ...
    if limit == -1:
        ...
        while True:
            ...
            res, card = contactSource(query_copy, server)
            numrequ += 1
            if card == -2:
                limit = limit / 2
                if limit == 0:
        ...
        reslist, card = contactSource(RDFS_RANGES, server)
...
def find_instance_range(server, t, p, limit=-1):
    ...
    if limit == -1:
        ...
        reslist, card = contactSource(INSTANCE_RANGES, server)
...
def get_concepts(endpoint, limit=-1):
    ...
    if limit == -1:
        ...
        while True:
            ...
            res, card = contactSource(query_copy, endpoint)
        ...
        reslist, card = contactSource(query, endpoint)
    ...
    for r in reslist:
        ...
        t = r['t']
        print(t, '\n', 'getting predicates ...')
        preds = get_predicates(endpoint, t)
        #print 'getting ranges ...'
        for p in preds:
            rn = {"t": t}
            pred = p['p']
            rn['p'] = pred
            rn['range'] = get_rdfs_ranges(endpoint, pred)
            rn['r'] = find_instance_range(endpoint, t, pred)
            results.append(rn)

    return results


def get_predicates(server, t, limit=-1):
    """
    ...
    if limit == -1:
        ...
        while True:
            ...
            if card == -2:
                ...
                if limit == 0:
                    rand_inst_res = get_preds_of_random_instances(server, t)
        ...
        reslist, card = contactSource(query, server)

    return reslist


def get_preds_of_random_instances(server, t, limit=-1):
    """
    ...
    if limit == -1:
        ...
        while True:
            ...
            if card > 0:
                ...
                inst = res[rand]
                inst_res = get_preds_of_instance(server, inst['s'])
...
def get_preds_of_instance(server, inst, limit=-1):
...
def getResults(query, endpoint, limit=-1):
    ...
    for r in reslist:
        ...
        if True in ifmetas:
            ...
            continue
        p = r['p']
        r['range'] = get_rdfs_ranges(endpoint, p)
        r['r'] = find_instance_range(endpoint, r['t'], p)
...
def contactSource(qeury, server):
    import requests

    js = "application/sparql-results+json"
    params = {'query': qeury, 'format': js}
    ...
    try:
        ...
        print ("Exception during query execution to", server, ': ', e)
    return None, -2
...
def get_external_links(endpoint1, rootType, pred, endpoint2, rdfmt2):
    ...
    while True:
        query_copy = query + " LIMIT " + str(limit) + " OFFSET " + str(offset)
        res, card = contactSource(query_copy, endpoint1)
        numrequ += 1
        if card == -2:
            limit = limit / 2
            if limit == 0:
...
def link_exist(s, c, endpoint):
    ...
    res, card = contactSource(query, server)
    if card > 0:
        #if res[0]:
...
def get_single_source_rdfmts(enpointmaps):
    ...
    for endpoint in enpointmaps:
        ...
        try:
            ...
            print("exception while writing single source molecules:", endpoint, e)
...
if __name__ == "__main__":
    ...
    #with open('../templates/dbpedia-only-template.json', 'w+') as f:
